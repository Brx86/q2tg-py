...
raw_conf = toml.load(base_dir / "config.toml")
raw_conf["forward"]["u"].update({v: k for k, v in raw_conf["forward"]["u"].items()})
raw_conf["forward"]["g"].update({v: k for k, v in raw_conf["forward"]["g"].items()})
raw_conf["forward"]["a"] = raw_conf["forward"]["u"] | raw_conf["forward"]["g"]
conf = Config.parse_obj(raw_conf)
logger.remove()
logger.add(sys.stderr, colorize=True, format=conf.log_format, level=conf.log_level)
...
class Msg:
    ...
    @staticmethod
    def reply(msgid: int) -> dict:
        # https://github.com/botuniverse/onebot-11/blob/master/message/segment.md#%E7%BA%AF%E6%96%87%E6%9C%AC
        return {"type": "reply", "data": {"id": msgid}}
    ...
    @staticmethod
    def video(file: str, cache: bool = True) -> dict:
        # https://github.com/botuniverse/onebot-11/blob/master/message/segment.md#%E5%9B%BE%E7%89%87
        return {"type": "video", "data": {"file": file, "cache": cache}}
